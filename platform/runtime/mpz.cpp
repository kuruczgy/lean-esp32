...
namespace lean {
static void *mpz_alloc(size_t size) { return malloc(size); }

static void mpz_dealloc(void *ptr, size_t size) { free(ptr); }
...
void mpz::init_uint64(uint64 v) {
  ...
  if (v <= std::numeric_limits<unsigned>::max()) {
    ...
    static_assert(sizeof(uint64) == 2 * sizeof(unsigned),
                  "unsigned should be half the size of an uint64");
    allocate(2);
...
mpz::mpz(char const *v) { init_str(v); }

mpz::mpz(unsigned int v) { init_uint(v); }

mpz::mpz(int v) { init_int(v); }

mpz::mpz(uint64 v) { init_uint64(v); }

mpz::mpz(int64 v) { init_int64(v); }

mpz::mpz(mpz const &s) { init_mpz(s); }

mpz::mpz(mpz &&s) : m_sign(s.m_sign), m_size(s.m_size), m_digits(s.m_digits) {
  s.m_digits = nullptr;
}
...
bool mpz::is_int() const {
  if (m_sign) {
    return m_size == 1 && m_digits[0] <= -static_cast<unsigned>(
...
bool mpz::is_unsigned_int() const { return m_size == 1 && !m_sign; }
...
size_t mpz::get_size_t() const {
  ...
  if (sizeof(size_t) == 8) {
    if (m_size == 1)
      ...
      return m_digits[0] +
             (static_cast<size_t>(m_digits[1]) << 8 * sizeof(mpn_digit));
  } else {
    return m_digits[0];
  }
}
...
mpz &mpz::operator+=(unsigned u) { return add(false, 1, &u); }
...
mpz &mpz::operator-=(unsigned u) { return add(true, 1, &u); }
...
mpz &mpz::operator*=(unsigned u) { return mul(false, 1, &u); }
...
mpz &mpz::operator/=(unsigned u) { return div(false, 1, &u); }

mpz &mpz::operator%=(mpz const &o) { return rem(o.m_size, o.m_digits); }
...
mpz mpz::ediv(mpz const &n, mpz const &d) {
  if (d.m_size > n.m_size) {
    ...
    q1.ensure_capacity(q_sz);
    r1.ensure_capacity(r_sz);
    mpn_div(n.m_digits, n.m_size, d.m_digits, d.m_size, q1.begin(), r1.begin());
    mpz q;
    q.set(q_sz, q1.begin());
...
unsigned mpz::mod8() const { return m_digits[0] & 0xFFu; }

unsigned mpz::mod16() const { return m_digits[0] & 0xFFFFu; }

unsigned mpz::mod32() const { return m_digits[0]; }
...
uint64 mpz::mod64() const {
  if (m_size == 1)
    ...
    return m_digits[0] +
           (static_cast<uint64>(m_digits[1]) << 8 * sizeof(mpn_digit));
}
