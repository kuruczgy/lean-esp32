...
static_assert(sizeof(mpn_double_digit) == 2 * sizeof(mpn_digit),
              "size alignment");

static const mpn_digit zero = 0;

int mpn_compare(mpn_digit const *a, size_t const lnga, mpn_digit const *b,
                size_t const lngb) {
  int res = 0;
...
void mpn_add(mpn_digit const *a, size_t const lnga, mpn_digit const *b,
             size_t const lngb, mpn_digit *c, size_t const lngc_alloc,
             size_t *plngc) {
  ...
  c[len] = k;
  size_t &os = *plngc;
  for (os = len + 1; os > 1 && c[os - 1] == 0;)
    os--;
  lean_assert(os > 0 && os <= len + 1);
}

void mpn_sub(mpn_digit const *a, size_t const lnga, mpn_digit const *b,
             size_t const lngb, mpn_digit *c, mpn_digit *pborrow) {
...
void mpn_mul(mpn_digit const *a, size_t const lnga, mpn_digit const *b,
             size_t const lngb, mpn_digit *c) {
    ...
    if (v_j == 0) { // This branch may be omitted according to Knuth.
      ...
      for (i = 0; i < lnga; i++) {
        ...
        t = ((mpn_double_digit)u_i * (mpn_double_digit)v_j) +
            (mpn_double_digit)c[i + j] + (mpn_double_digit)k;
...
static size_t div_normalize(mpn_digit const *numer, size_t const lnum,
                            ...
                            mpn_buffer &n_numer, mpn_buffer &n_denom) {
  size_t d = 0;
  while (lden > 0 && ((denom[lden - 1] << d) & MASK_FIRST) == 0)
    d++;
...
static void div_1(mpn_buffer &numer, mpn_digit const denom, mpn_digit *quot) {
  mpn_double_digit q_hat, temp, ms;
  mpn_digit borrow;

  for (size_t j = numer.size() - 1; j > 0; j--) {
    temp = (((mpn_double_digit)numer[j]) << DIGIT_BITS) |
           ((mpn_double_digit)numer[j - 1]);
...
static void div_n(mpn_buffer &numer, mpn_buffer const &denom, mpn_digit *quot,
                  mpn_digit *rem, mpn_buffer &ms, mpn_buffer &ab) {
  ...
  for (size_t j = m - 1; j != (size_t)-1; j--) {
    temp = (((mpn_double_digit)numer[j + n]) << DIGIT_BITS) |
           ((mpn_double_digit)numer[j + n - 1]);
...
void mpn_div(mpn_digit const *numer, size_t const lnum, mpn_digit const *denom,
             size_t const lden, mpn_digit *quot, mpn_digit *rem) {
