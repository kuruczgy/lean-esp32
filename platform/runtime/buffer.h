...
/**
    ...
    temporary storage.
*/
template <typename T, size_t INITIAL_SIZE = 16> class buffer {
protected:
  T *m_buffer;
  size_t m_pos;
  size_t m_capacity;
  ...
  void expand() { set_capacity(m_capacity << 1); }
...
public:
  ...
  buffer()
      : m_buffer(reinterpret_cast<T *>(m_initial_buffer)), m_pos(0),
  ...
  bool operator!=(buffer const &other) const { return !operator==(other); }

  T const &back() const {
    lean_assert(!empty() && m_pos > 0);
  ...
  template <typename C> void append(C const &c) { append(c.size(), c.data()); }
