{
  ...
  outputs = { self, nixpkgs }:
    let
      # Only tested on x86_64-linux, but I don't see a reason it wouldn't work
      # out of the box on anything else.
      system = "x86_64-linux";
          ...
          (self: super: {
            picolibc = self.callPackage picolibc-pkg {
              stdenv = self.overrideCC self.stdenv
                (self.buildPackages.llvmPackages_18.tools.clangNoLibc.override (old: {
                  # TODO: can't we just use NIX_CFLAGS_COMPILE here instead?
                  extraBuildCommands = old.extraBuildCommands + ''
                    echo "-Qunused-arguments" >> $out/nix-support/cc-cflags
                    echo "-nodefaultlibs" >> $out/nix-support/cc-cflags
                  '';
                }));
            };
        ...
        pkgs.lib.mapAttrs (k: v: { type = "app"; program = "${pkgs.writeScript k v}"; }) {
          ...
          qemu = ''
            ...
            ${self.packages.${system}.qemu}/bin/qemu-system-riscv32 \
              ...
              -drive file=./platform/efuse.bin,if=none,format=raw,id=efuse \
